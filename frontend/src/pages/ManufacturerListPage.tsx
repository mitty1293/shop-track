...
const ManufacturerListPage: React.FC = () => {
    ...
    const filteredManufacturers = useMemo(() => {
        ...
        if (!filterName) return manufacturers; // フィルターが空なら全件返す
        return manufacturers.filter(manufacturer =>
            manufacturer.name.toLowerCase().includes(filterName.toLowerCase()) // 大文字・小文字を区別しない部分一致
        );
    }, [manufacturers, filterName]); // manufacturers または filterName が変更された時のみ再計算
    ...
    const paginatedManufacturers = filteredManufacturers
        ? (rowsPerPage === -1 // "All" が選択されたかチェック
            ? filteredManufacturers // "All" なら元の配列をそのまま使用
            : filteredManufacturers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) // それ以外ならスライス
            )
        : [];
    ...
    return (
        <Container maxWidth="lg" sx={{ mt: 2 }}>
            ...
            {(!manufacturers) ? (
                <Typography>Loading data or no manufacturers available.</Typography>
            ) : (!filteredManufacturers || filteredManufacturers.length === 0) ? (
                <Typography>No manufacturers match your filter criteria "{filterName}".</Typography>
            ) : (
                <Paper sx={{ width: '100%', overflow: 'hidden' }}>
                    ...
                    <TablePagination
                        ...
                        component="div"
                        count={filteredManufacturers.length || 0}
